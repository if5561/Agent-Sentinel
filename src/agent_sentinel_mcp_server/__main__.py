...
async def main() -> None:
    # 方法说明：初始化日志并启动 MCP 服务主循环，让外部 Agent 可以通过标准输入输出调用工具。
...
class AgentSentinelMCPServer:
    def __init__(self) -> None:
        # 方法说明：读取项目配置，后续工具调用会用这些配置连接日志和指标后端。
        self._settings = get_settings()

    async def serve(self) -> None:
        # 方法说明：持续读取 MCP 客户端请求，处理后按 JSON-RPC 协议写回响应。
        while True:
    ...
    async def _handle_request(self, request: dict[str, Any]) -> dict[str, Any] | None:
        # 方法说明：按 MCP 方法名分发请求，支持初始化、工具列表和工具调用三类消息。
    ...
    async def _call_tool(self, params: dict[str, Any]) -> dict[str, Any]:
        # 方法说明：根据工具名称调用对应的数据查询逻辑，并包装成 MCP 要求的返回格式。
...
async def query_aliyun_sls(arguments: dict[str, Any]) -> dict[str, Any]:
    # 方法说明：通过 MCP 查询阿里云 SLS 日志，为告警诊断补充原始日志证据。
...
async def query_prometheus(arguments: dict[str, Any]) -> dict[str, Any]:
    # 方法说明：通过 MCP 查询 Prometheus 指标，支持显式 PromQL，也能按告警摘要生成默认查询。
    base_url = (
...
def _build_default_promql(alert_summary: str) -> str:
    # 方法说明：根据告警摘要中的关键词选择一个基础 PromQL，给未传查询语句的调用兜底。
...
def _summarize_prometheus_result(result: Any) -> str:
    # 方法说明：把 Prometheus 原始返回压缩成一句摘要，方便模型快速判断是否查到时间序列。
...
def _tool_definitions() -> list[dict[str, Any]]:
    # 方法说明：声明 MCP 对外暴露的工具名称、用途和入参结构，客户端会据此展示可调用能力。
    return [
        {
...
async def _read_message() -> dict[str, Any] | None:
    # 方法说明：按 MCP 的 Content-Length 协议从标准输入读取一条完整 JSON 请求。
...
async def _write_message(payload: dict[str, Any]) -> None:
    # 方法说明：把 JSON-RPC 响应按 MCP 的 Content-Length 协议写回标准输出。
...
def _error_response(request_id: Any, code: int, message: str) -> dict[str, Any]:
    # 方法说明：生成标准 JSON-RPC 错误响应，让客户端能明确知道失败原因。
    return {"jsonrpc": "2.0", "id": request_id, "error": {"code": code, "message": message}}


def _str_or_none(value: Any) -> str | None:
    # 方法说明：把可选参数统一转换成非空字符串；空值统一返回 None 方便后续兜底。
    if value is None:
        return None
